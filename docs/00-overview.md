...
`monitor` **mirror** these contracts (generated or hand-kept) - see `01-contracts.md` SCross-language.
...
1. **Self-healer is Node + Claude**, implemented in `services/generator`, *not* in the Go monitor.
   The Go monitor only *detects* drift and *enqueues* a job. See `services/generator.md` SSelf-healer.
