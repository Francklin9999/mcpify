...
> ⚠️ **The monitor targets the SOURCE website/API a server was generated from — never user-deployed
...
> ⚠️ This service only **detects and enqueues**. All healing/codegen/Claude is in `services/generator`.
...
- **Output:** `health_events` rows; `servers.status`/`confidence` updates; **queue messages**
  `SelfHealJob` / `RegenerateJob` (`01 §4`). It is the **only producer of those two job kinds**.
- Mirrors only `01 §4` (queue) and `01 §5` (registry rows) as Go structs (`01 §Cross-language`).
...
- Write `health_events` (pass/fail + `error_class` matching `ToolFailure.errorClass`, `01 §4`).
- On `fail` for a specific tool ⇒ enqueue `SelfHealJob { serverId, toolName, failure }`.
- Pass/fail nudges `servers.confidence` within [0,1] per `01 §5` confidence rules.
...
1. Go structs for `01 §4` job payloads + `01 §5` rows.
...
## Acceptance criteria
- A simulated tool failure produces exactly one well-formed `SelfHealJob` (`01 §4`).
- A large DOM change produces a `RegenerateJob` and flips status to `regenerating`.
- Confidence updates stay within [0,1] and follow `01 §5`.
