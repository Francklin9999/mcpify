...
## Purpose
Fetch a target URL and produce a **`CaptureBundle`** (`01-contracts.md §1`) — a rendered DOM snapshot
...
- **Output:** `CaptureBundle` (`01 §1`). `source: 'scraper'`, `tier` set to the tier that succeeded.
- Mirrors `CaptureBundle` + `NetworkCapture` as **pydantic** models (`01 §Cross-language`).
...
`dom.selectorsOfInterest: ElementRef[]` (`01 §1`) with `role` + `selector` + `fallbackSelectors`. This
