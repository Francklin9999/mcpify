...
> (!) The **self-healer lives here (Node + Claude)**, not in the Go monitor. The monitor only detects and
> enqueues; this service does all Claude/codegen work. (Corrects the source diagram.)

## Inputs / Outputs
| Module | Input | Output |
|--------|-------|--------|
| Inference | `CaptureBundle` (`01 S1`) | `InferenceResult` / `ToolDefinition[]` (`01 S2`) |
| Codegen | `ToolDefinition[]` | `GeneratedServerArtifact` (`01 S3`) |
| Self-healer | `SelfHealJob` / `RegenerateJob` (`01 S4`) | new `ServerVersion` (`01 S5`, `02`) |
...
3. **zod-validate** each tool; drop/repair invalid ones. Compute aggregate `confidence` (`01 S5` rules).
...
2. Emit `claude_desktop_config.json` snippet + README. Bundle as `GeneratedServerArtifact` (`01 S3`).
...
- **`self_heal`:** given `ToolFailure` (`01 S4`), re-snapshot **only the failing tool's page** (via
...
- Artifact lands in R2 and rows land in Postgres matching `01 S3/S5`.
...
- **Adversarial fuzz** (21 cases): hostile form field names (quotes, `${}`, `__proto__`, newlines, unicode),
  100-field forms, weird HTTP methods, 5x`{id}` paths, 200-call captures, name collisions, odd URLs, empty
