...
```
User (web app)
  │  POST /api/generate { url, legalMode }                         (01 §7)
  ▼
web API ── enqueue GenerateJob ──►  Redis/BullMQ  (mcp-jobs)       (01 §4)
  │  returns { jobId }
  ▼
...
scraper  (Python, 3 tiers)
  ...
  │  produces CaptureBundle  (01 §1)  ── DOM + NetworkCapture[]
  ▼
generator: inference (Claude API)
  │  CaptureBundle → InferenceResult { tools[], confidence }       (01 §2)
  │  zod-validate every ToolDefinition; drop invalid
  ▼
generator: codegen (@modelcontextprotocol/sdk)
  │  ToolDefinition[] → GeneratedServerArtifact (server.ts + config) (01 §3)
  │  upload artifact → R2 ; write servers/server_versions/tools → Postgres (02)
  │  cache result → job:{jobId} (Redis)
  ▼
web: GET /api/jobs/:jobId → { status, result?, error? }   status ∈ queued|running|done|failed
...
monitor (Go, goroutines, never opens a browser)
  ...
  ├─ tool failed → enqueue SelfHealJob { serverId, toolName, failure }      (01 §4)
  └─ large diff  → enqueue RegenerateJob { serverId, reason:'large_drift' } (01 §4)
        │
        ▼
...
User browses normally (side panel open)
  ...
  ▼
extension builds a CaptureBundle { source:'extension' }            (01 §1, same shape as scraper)
  │  legal gate (04): session mode, user's own account, NEVER persist credentials/raw secrets
  ▼
POST /api/servers/:id/contribute { CaptureBundle }                 (01 §7)
