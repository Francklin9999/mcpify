...
- **Net-intercept output:** `CaptureBundle { source:'extension' }` — **identical shape to the scraper's
  output** (`01 §1`). This is the decoupling: the generator can't tell where a bundle came from.
- Calls the Web API (`01 §7`) only — `POST /api/generate`, `POST /api/servers/:id/contribute`. Never
...
- Chat goes through the **Web API** via a dedicated assist route — **add `POST /api/assist`** to the
  surface in `01 §7` (request: `{ messages, pageContext, availableTools }` → streamed assistant turn).
...
- On each turn, build a **compact page context** from the live tab: visible text outline (headings/labels),
  the `ElementRef[]` of actionable elements (reuse the `01 §1` `ElementRef` shape), current URL, and a
...
- An accepted suggestion maps to a `BrowserStep[]` (`01 §2`) executed in the live tab via
...
> `ExecutionStrategy` note in `01 §2`. Authenticated session **execution** is a deliberate post-v1 extension.
...
## Data touchpoints
- Local only (chrome.storage for history/state). Server interaction is exclusively via `01 §7` routes.
...
- Assert: assembled `CaptureBundle` validates against the `01 §1` schema and is byte-identical in shape to
...
- ~~Exactly how `session`-mode requests are replayed by generated tools~~ — **resolved: out of v1 scope**
  (public-only execution; see `01 §2`). Revisit when designing post-v1 session execution.
