...
The server-side capture source; the extension net-intercept produces the same `CaptureBundle` shape (`01 §1`).

## Layout

| Module | Role |
|--------|------|
| `contracts.py` | pydantic mirror of `CaptureBundle`/`NetworkCapture`/`ElementRef` (`01 §1`), with the **fail-closed** secret-header rejection mirroring the TS `superRefine`. |
