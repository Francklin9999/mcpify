...
export function createServer(deps: CreateServerDeps = {}): McpServer {
  ...
  register(
    "get_product",
    { description: "Fetch a product by id", inputSchema: { "id": z.string() } },
    async (args) => callHttp({"method":"GET","urlTemplate":"https://example.com/api/products/{id}","headers":{"accept":"application/json"},"paramMapping":{"id":{"in":"path","key":"id"}}}, args),
  );

  register(
    "search_products",
    { description: "Search products", inputSchema: { "q": z.string(), "limit": z.number().optional(), "payload": z.string().optional(), "xtoken": z.string().optional() } },
    async (args) => callHttp({"method":"POST","urlTemplate":"https://example.com/api/search","headers":{"accept":"application/json"},"paramMapping":{"q":{"in":"query","key":"q"},"limit":{"in":"query","key":"limit"},"payload":{"in":"body","key":"data"},"xtoken":{"in":"header","key":"x-token"}}}, args),
  );

  register(
    "get_product_details",
    { description: "Open a product page in a browser and return structured JSON details.", inputSchema: { "asin": z.string() } },
    async (args) => callBrowser({"steps":[{"action":"navigate","value":"https://example.com/dp/{{asin}}"},{"action":"waitFor","target":{"role":"page","selector":"body"}},{"action":"extract","value":"json:product"}]}, args, browsing),
  );

  register(
    "browser_navigate",
    { description: "Navigate the shared browser session to a URL (absolute, or relative to this site). Starts the session if needed. Returns a snapshot of the new page's interactive elements.", inputSchema: { url: z.string() } },
    async (args) => guardBrowsing(() => browsing.navigate(String(args.url))),
  );

  register(
    "browser_snapshot",
    { description: "List the current page's interactive elements, each with a [ref] for browser_click/browser_type/browser_select_option, plus the page title, URL and a visible-text excerpt. Call this first, and again after any action — refs change whenever the page changes.", inputSchema: {} },
    async (args) => guardBrowsing(() => browsing.snapshot()),
  );

  register(
    "browser_click",
    { description: "Click the element with the given [ref] from the latest browser_snapshot (a link, button, result, 'Add to cart', pagination, etc.). Returns a fresh snapshot after the click.", inputSchema: { ref: z.string() } },
    async (args) => guardBrowsing(() => browsing.click(String(args.ref))),
  );

  register(
    "browser_type",
    { description: "Type text into the input/textarea with the given [ref]. Set submit=true to press Enter afterwards (e.g. to run a search). Returns a fresh snapshot.", inputSchema: { ref: z.string(), text: z.string(), submit: z.boolean().optional() } },
    async (args) => guardBrowsing(() => browsing.type(String(args.ref), String(args.text), args.submit === true)),
  );

  register(
    "browser_press_key",
    { description: "Press a key (Enter, Escape, Tab, ArrowDown, ArrowUp, ArrowLeft, ArrowRight, Backspace). Optionally target a [ref]; otherwise the focused element. Returns a fresh snapshot.", inputSchema: { key: z.string(), ref: z.string().optional() } },
    async (args) => guardBrowsing(() => browsing.pressKey(String(args.key), args.ref ? String(args.ref) : undefined)),
  );

  register(
    "browser_select_option",
    { description: "Choose an option (by value or visible label) in the <select> with the given [ref]. Returns a fresh snapshot.", inputSchema: { ref: z.string(), value: z.string() } },
    async (args) => guardBrowsing(() => browsing.selectOption(String(args.ref), String(args.value))),
  );

  register(
    "browser_back",
    { description: "Go back to the previous page in the shared browser session. Returns a fresh snapshot.", inputSchema: {} },
    async (args) => guardBrowsing(() => browsing.back()),
  );

  register(
    "browser_read_page",
    { description: "Return the readable text content of the current page in the shared browser session (no markup).", inputSchema: {} },
    async (args) => guardBrowsing(() => browsing.read()),
  );

  register(
    "browser_extract",
    { description: "Extract structured JSON from the current page. mode = 'product' (price/availability/rating/...), 'listing' (search-result cards), or 'metadata' (title/headings/links, the default).", inputSchema: { mode: z.string().optional() } },
    async (args) => guardBrowsing(() => browsing.extract(String(args.mode || "metadata"))),
  );
