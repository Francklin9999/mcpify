...
| `FORGE_AUTH_HANDOFF` | `1` | When max stealth still hits a sign-in/CAPTCHA wall, open a VISIBLE browser, tell the user, and wait for them to sign in / solve it — then continue capturing the authenticated page in the same session. Needs a display. `0` disables. |
...
| `FORGE_BROWSER_BACKEND` | *(off)* | Set `extension` to route capture through the **urlmcp Chrome extension** running in your everyday browser (no flags, no relaunch). Run `npx urlmcp install-extension` first. Degrades to the managed browser when the extension isn't connected. |
