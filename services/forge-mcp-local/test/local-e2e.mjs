...
try {
  ...
  await withServer({ MCP_FORGE_HOME: home1 }, async (call) => {
  ...
  await withServer(
    { MCP_FORGE_HOME: home2, FORGE_INFERENCE: "openai-compatible", FORGE_OPENAI_BASE_URL: openaiBase, FORGE_API_KEY: "test", FORGE_MODEL: "mock-model" },
    async (call) => {
      const gen = await call("forge_generate", { url: siteUrl });
