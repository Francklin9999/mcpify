...
try {
  ...
  // Pin the in-process managed scraper, headless: FORGE_USE_REAL_BROWSER=0 keeps the deterministic pipeline from
  // auto-opening the user's real Chrome, MCP_BROWSER_HEADLESS=1 keeps the STEALTH-MAX fallback from opening a headful
  // window on a desktop, and FORGE_CRAWL=0 asserts single-page capture (no base-domain exploration).
  await withServer({ URLMCP_HOME: home1, FORGE_ALLOW_PRIVATE_HOSTS: "1", FORGE_AUTH_HANDOFF: "0", FORGE_USE_REAL_BROWSER: "0", MCP_BROWSER_HEADLESS: "1", FORGE_CRAWL: "0" }, async (call) => {
  ...
  await withServer(
    { URLMCP_HOME: home2, FORGE_ALLOW_PRIVATE_HOSTS: "1", FORGE_AUTH_HANDOFF: "0", FORGE_USE_REAL_BROWSER: "0", MCP_BROWSER_HEADLESS: "1", FORGE_CRAWL: "0", FORGE_INFERENCE: "openai-compatible", FORGE_OPENAI_BASE_URL: openaiBase, FORGE_API_KEY: "test", FORGE_MODEL: "mock-model" },
    async (call) => {
      const gen = await call("forge_generate", { url: siteUrl });
